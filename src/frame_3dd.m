...
function frame_3dd(XYZ,ELT,RCT,EAIJ,P,U,D, IOfilename)
...
% *** Please email extensions and enhancements of this function to me. ***
  ...
  shear = 1;		     % 1: include shear deformation effects, 0: don't
  geom  = 1;		     % 1: include geometric stiffness effects, 0: don't
  exagg = 5;		     % exaggeration factor for Gnuplot output
  scale = 0.1;		     % zoom scale factor for Gnuplot plotting
...
% run the frame3dd analysis
  if ismac || isunix
  system(['./frame3dd -q -i IO' IOfilename ' > nul']);
  else
  system(['frame3dd -q -i IO' IOfilename]);
  end   
